...
def check_and_load(file):
    ...
    for field, pattern in literals.regex_course.items():
        assert re.match(pattern, info[field]), f"Invalid '{field}': '{info[field]}' in {file}"
        
    assert info['n_sections'] > 0, "Number of sections must be positive"
    if missing := info['missing_sections']:
        assert set(missing).issubset(range(1, info['n_sections'])), "Missing sections that don't exist"


    # sheets info check
    for sheet in ['enrolment', 'marks']:
        if not info[sheet]:    
            print(f"{sheet.title()} sheet ID is not specified in {file}, creating a new one...")
...
def create_from_template(sheet, info):
    ...
    folder_id = input("Enter the Google Drive folder ID where the new sheet will be created: ")
