...
import discord, vars, literals, re
...
class VerificationButtonView(View):
    def __init__(self, *args, **kwargs):
        ...
        self.timeout = None
        self.add_item(OpenFormButton(label="Verify Me", form_to_open=studen_id_form))
...
async def check_student(input_text, member):
    ...
    if advising_id != "" and advising_id != member.id:
        ...
        if member.id in vars.df_student["Discord ID (Adv. Verified)"]:
            conflict_id = vars.df_student[vars.df_student["Discord ID (Adv. Verified)"] == member.id].index[0]
