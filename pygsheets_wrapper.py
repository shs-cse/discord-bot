...
def copy_sheet(template_id, title, folder_id, set_sheet_cells=None):
    google_client = pygs.authorize(client_secret='credentials.json')
    template = google_client.open_by_key(template_id)
    spreadsheet = google_client.create(
        title=title, template=template, folder=folder_id)
    # populate values if provided
    if set_sheet_cells:
        for sheet_name, set_values in set_sheet_cells.items():
            if sheet_name: # should use try except instead
